...
const CUT: Action<CustomActions> = m(&[LCtrl, X]);
...
pub static LAYERS: keyberon::layout::Layers<CustomActions> = &[
    ...
    */
    &[
        ...
        &[CUT,     k(CapsLock), k(A),    k(S),    k(D),     k(F),   k(G),     k(H),   k(J),   k(K),     k(L),     k(SColon), k(Quote),    k(No),       k(Enter),  k(PgUp)],
        &[COPY,    k(LShift),   k(Z),    k(X),    k(C),     k(V),   k(B),     k(N),   k(M),   k(Comma), k(Dot),   k(Slash),  k(LShift),   k(No),       k(Up),     k(PgDown)],
        &[PASTE,   k(LCtrl),    k(LGui), k(LAlt), k(No),    k(No),  k(Space), k(No),  k(No),  k(RAlt),  k(RCtrl), l(1),      k(Left),     k(No),       k(Down),   k(Right)],
    ], 
    /* misc layer */
    &[
        &[k(No),           k(Grave), k(F1), k(F2), k(F3), k(F4), k(F5), k(F6), k(F7), k(F8), k(F9), k(F10), k(F11), k(F12), k(Delete), BOOTLOADER],
        &[k(No),           Trans,    Trans, Trans, Trans, Trans, Trans, Trans, Trans, Trans, Trans, Trans,  Trans,  Trans,  Trans,     UNDERGLOW],
    ...
    &[
        ...
        &[CUT,     k(CapsLock), k(A),      k(O),     k(E),     k(U),     k(I),     k(D),     k(H),     k(T),     k(N),     k(S),   k(Minus),    k(No),       k(Enter),  k(PgUp)],

...
pub static LAYERS: keyberon::layout::Layers = &[
    &[
        ...
        &[k(Copy),    k(LShift),   k(Z),    k(X),    k(C),     k(V),     k(B),     k(N),     k(M),     k(Comma), k(Dot),   k(Slash),  k(LShift),   k(No),       k(Up),     k(PgDown)],
        &[k(Paste),   k(LCtrl),    k(LGui), k(LAlt), k(No),    k(No),    k(Space), k(No),    k(No),    k(RAlt),  k(RCtrl),  k(RGui),  k(Left),     k(No),       k(Down),   k(Right)],
    ], 
    &[
        &[k(No),      k(Escape),   k(F1),   k(F2),   k(F3),    k(F4),    k(F5),    k(F6),    k(F7),    k(F8),    k(F9),     k(F10),    k(F11),      k(F12),      k(BSpace), k(Application)],
